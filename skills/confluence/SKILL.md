...
Commands targeting a page accept its ID or page URL. Comment update/delete
require the comment ID or its permalink; attachment download/update/delete
require the attachment ID, never a URL. Discover those IDs with `comment list`
or `attachment list` on the page. If the user supplies only a name or topic,
search first; do not guess the target.
...
confluence-cli page watch <id|url>        # subscribe to page notifications
confluence-cli page unwatch <id|url>      # unsubscribe
confluence-cli page watch-status <id|url> # check subscription status
...
confluence-cli comment add <id|url>       # post a comment (--dry-run previews)
...
For edits, fetch the full raw storage body and its version before preparing a
replacement; a section or rendered summary is not a safe editing source. Keep
human prose and unrelated rich content intact. Follow
[writing-pages.md](references/writing-pages.md#update) for versioned updates.

Use the user's existing authorization for the requested change. Preparing a
draft or running `--dry-run` does not require another approval; preserve the
specific human-comment gate below. Report the answer or completed change with
the relevant page link and supporting section/version. State limited search or
history coverage plainly; do not present a partial result as exhaustive.
...
fetch every page when exhaustive coverage is needed (it resumes from `--cursor`
when both are set), or `--limit N` to size each request. For very large outputs
use `--format ndjson` (one JSON object per line, items only).
...
A reply is posted under the user's name. Help them decide what to say to the
person who asked by preparing the relevant facts and a draft.
...
- **Bot or agent counterparts** do not need the per-comment gate — use existing
  authorization to reply and keep attribution.

Full protocol: [replying-to-people.md](references/replying-to-people.md).

## AI attribution (agent writes)

Mark content you author with one AI attribution link. For existing pages,
identify the AI-edited sections and preserve the human's content; do not label
their whole page as AI-generated. A title-only update keeps the body unchanged.
Human-authored text, including a reply the user writes or rewrites, is posted
verbatim without an AI marker. Keep attribution once per page/comment.

- **Pages** — use an Info macro for storage or the format's native link syntax
  for markdown/wiki. See
  [writing-pages.md](references/writing-pages.md).
