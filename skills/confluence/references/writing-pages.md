...
- **storage body** (`--body-format storage`) — prepend an Info macro:
  ...
  <ac:structured-macro ac:name="info">
    <ac:rich-text-body>
      <p>[AI] 本页面由 AI 通过 <a href="https://angelmsger.github.io/confluence-cli/">confluence-cli</a> 生成。</p>
    </ac:rich-text-body>
  </ac:structured-macro>
  ```
...
- **markdown / wiki body** — prepend a single callout line (markdown can't carry the
  ...
  ```markdown
  > [AI] 本页面由 AI 通过 [confluence-cli](https://angelmsger.github.io/confluence-cli/) 生成。
  ```

**Language** — write the sentence in the same language as the page content (the
user's language); keep the `[AI]` marker, the URL, and the `confluence-cli` label
constant. The marker is plain-ASCII `[AI]`, **never an emoji** — some Data Center
databases (e.g. MySQL `utf8mb3`) can't store 4-byte characters and would reject or
truncate the page body:

| lang | banner sentence |
|------|-----------------|
| zh | `[AI] 本页面由 AI 通过 [confluence-cli](…) 生成。` |
| en | `[AI] This page was generated by AI via [confluence-cli](…).` |
| ja | `[AI] このページは AI が [confluence-cli](…) を通じて生成しました。` |
